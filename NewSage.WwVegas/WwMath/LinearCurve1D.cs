// -----------------------------------------------------------------------
// <copyright file="LinearCurve1D.cs" company="NewSage">
...
{
    private const uint ChunkCurve1D = 0x0002_0657;

    public override PersistFactory Factory => new SimplePersistFactory<LinearCurve1D>(MathIds.ChunkIdLinearCurve1D);
    ...
    public override bool Save(ChunkSave cSave)
    {
        ...
        _ = cSave.BeginChunk(ChunkCurve1D);
        _ = base.Save(cSave);
        _ = cSave.EndChunk();
        return true;
    }
    ...
    public override bool Load(ChunkLoad cLoad)
    {
        ...
        while (cLoad.OpenChunk())
        {
            switch (cLoad.CurrentChunkId)
            {
                ...
                default:
                    Debug.WriteLine($"Unhandled chunk ID: 0x{cLoad.CurrentChunkId:X8}");
                    break;
            }
